...
// Send the conversion event to Partnerize
  ...
  requestUrl += '/campaign:' + data.campaignId;
  requestUrl += '/clickref:' +  getPartnerizeClickref();

  if (data.additionalParameters) {
    for (let i = 0; i < data.additionalParameters.length; i++) {
      let parameter = formatParameter(data.additionalParameters[i].name);
      requestUrl += '/' + parameter + ':' + data.additionalParameters[i].value;
    }
  }
